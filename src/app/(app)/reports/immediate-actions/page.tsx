...
import { useImmediateActionsReports } from '@/shared/hooks/report-hooks'
...
export default function ImmediateActionsReportsPage() {
  ...
  const handleExport = async (reportId: string, format: 'pdf' | 'docx') => {
    try {
      ...
      // await api.immediateActions.export(reportId, format)
                ...
                <TableBody>
                  {filteredReports.map((report) => (
                    <TableRow key={report.id} className="hover:bg-gray-50">
                      ...
                      <TableCell>
                        {report.fecha_inicio && report.fecha_termino ? (
                          <div className="text-sm">
                            <div>{format(new Date(report.fecha_inicio), 'dd/MM/yyyy', { locale: es })}</div>
